  ...
  const recommendations = [
    {
      ...
      status: nutrientLevels.some(
        (n) => Math.abs(n.value - n.recommended) > n.recommended * 0.2
      )
        ? "warning"
        : "success",
      message: nutrientLevels.some(
        (n) => Math.abs(n.value - n.recommended) > n.recommended * 0.2
      )
        ? "Some nutrients require attention"
        : "Nutrient levels are optimal",
      icon: nutrientLevels.some(
        (n) => Math.abs(n.value - n.recommended) > n.recommended * 0.2
      )
        ? AlertTriangle
        : Check,
    },
    {
      title: "Soil pH",
      status:
        physicalParameters.find((p) => p.name === "pH")?.value! < 6.5
          ? "warning"
          : physicalParameters.find((p) => p.name === "pH")?.value! > 7.5
          ? "warning"
          : "success",
      message:
        physicalParameters.find((p) => p.name === "pH")?.value! < 6.5
